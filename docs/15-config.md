...
| `FAP_AUTOUPDATE_MIN` | `0` *(tắt)* | **Tự cập nhật khi đang chạy**: số **phút** giữa mỗi lần bot/watcher tự dò `git pull` → `selftest` → nếu PASS thì tự khởi động lại nạp mã mới. `0`/trống = tắt (bot vẫn có lệnh `/update` thủ công). · **Update-while-running**: minutes between a resident bot/watcher's auto `git pull` → `selftest` → self-restart on pass; `0` = off (bots still have manual `/update`) | `FAP_AUTOUPDATE_MIN=180` |
