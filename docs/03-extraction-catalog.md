...
| `GetMarkByCourse` | Điểm THÀNH PHẦN một môn theo `CourseId`. **`data` là BẢNG HTML (chuỗi), KHÔNG phải JSON** — xác minh live: `<table>…<td>Progress test 1</td><td>10.0 %</td><td>9.7</td>…</table>` (cột `Grade category / Grade item / Weight / Value / Comment` + `tfoot` Average/Status). → parse HTML (stdlib) lấy được điểm thành phần **headless** (không cần browser/Cloudflare). | `campusCode`, `Authen`, `CourseId`, `checksum`, `rollNumber` *(có thể cần `SubjectCode`)* | HTML: `category`, `item`, `weight`, `value` + `Average`/`Status` | high |
