...
def _day_digest(sessions, day):
    ...
    for start, end, s in items:
        ...
        lines.append(f"🕐 {rng}  {s.get('subjectCode','')}  {fmt.room(s)}")
    return "\n".join(lines)
...
def _week_digest(sessions, today):
    ...
    for i in range(7):
        ...
        for start, end, s in day_items:
            lines.append(f"   🕐 {start.strftime('%H:%M')}  {s.get('subjectCode','')}  {fmt.room(s)}")
    if total == 0:
...
def run(cmd="test"):
    ...
    parts = str(cmd).split()
    name = {"weekly": "week"}.get(parts[0], parts[0]) if parts else "today"
